...
class Head2HeadMatches extends StatelessWidget {
  ...
  const Head2HeadMatches({Key? key, required this.isHomeTeam,
    required this.matchesStatus, required this.cubit}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(isHomeTeam==null?
        "${cubit.matchDetailsModel!.head2head!.homeTeam!.draws}":
          isHomeTeam==true?"${cubit.matchDetailsModel!.head2head!.homeTeam!.wins}":
...
class MatchInfoItem extends StatelessWidget {
  final String icon;
  final String text;
  const MatchInfoItem({Key? key, required this.icon, required this.text}) : super(key: key);
...
class MatchInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String refereeName =
    referee==true?cubit.matchDetailsModel!.match!.referees!.
    firstWhere((element) => element.role=="REFEREE").name!:"";
    return Column(
      children: [
        Row(
          children: [
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: MatchInfoItem(
                        ...
                        text: timeFormat(cubit.matchDetailsModel!.match!.utcDate!)
                    ),
                  ),
                  Expanded(
                    child: MatchInfoItem(
                        icon: "assets/images/calendar.png",
                        text: "Fixture ${cubit.matchDetailsModel!.match!.matchday}"
                    ),
                  ),
                ],
              ),
              ...
              Row(
                children: [
                  ...
                  if(referee)
                  Expanded(
                    child: DefaultSvgNetworkImage(
                        url: "${cubit.matchDetailsModel!.match!.competition!
                        .area!.ensignUrl}",
                        width: 25, height: 25),
                  )
                ],
...
class TeamBestPlayer extends StatelessWidget {
  ...
  const TeamBestPlayer({Key? key, required this.cubit, required this.leagueID,
    required this.homeTeam, required this.awayTeam}) : super(key: key);

  @override
  Widget build(BuildContext context) {

    Scorers homePlayer = cubit.scorers[leagueID]!.firstWhere((element) =>
    element.team!.id==homeTeam.id);

    Scorers awayPlayer = cubit.scorers[leagueID]!.firstWhere((element) =>
          ...
          children: [
            Container(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ImageIcon(const AssetImage('assets/images/matches.png'),
                    color: havan,
                    size: 25,),
...
class BestPlayer extends StatelessWidget {
  ...
  final String teamImage;
  final bool isHome;
  const BestPlayer({Key? key, required this.scorer, required this.teamImage,
    required this.isHome}) : super(key: key);
...
class TopScorerDetails extends StatelessWidget {
  ...
  bool? isGoals;
  TopScorerDetails({Key? key, required this.icon, required this.property,this.isGoals}) : super(key: key);
