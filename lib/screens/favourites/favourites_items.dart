...
class LeagueHead extends StatelessWidget {
  final String image;
  final String name;
  const LeagueHead({Key? key, required this.image, required this.name}) : super(key: key);
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              SizedBox(
                width: ligue1?50:40,height: ligue1?50:40,
...
class TeamsBuilder extends StatelessWidget {
  final Map<String,dynamic> league;
  const TeamsBuilder({Key? key, required this.league, }) : super(key: key);
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context,index)=>BuildTeamInGrid(
          team: league['teams'][index],index: index,),
...
class BuildTeamInGrid extends StatelessWidget {
  final Teams team;
  final int index;
  const BuildTeamInGrid({Key? key, required this.team,
    required this.index}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    KickCubit cubit = KickCubit.get(context);
    bool favourite = cubit.favouriteTeams.any((element) => element.id==team.id);
    return InkWell(
      onTap: (){
        favourite = cubit.favouriteTeams.any((element) => element.id==team.id);
        if(favourite){
          cubit.favouriteTeams.removeWhere((element) => element.id==team.id);
          cubit.removeFromFavourites(team: team);
        }else{
          cubit.favouriteTeams.add(team);
          cubit.addToFavourites(team: team);
        }
      },
      child: Container(
        decoration: BoxDecoration(
          border: Border.all(
              color: favourite?havan:grey,
              width: favourite?2:1
          ),
...
class FavouriteTeamPicAndName extends StatelessWidget {
  ...
  const FavouriteTeamPicAndName({Key? key ,required this.teamName, required this.teamImage}) : super(key: key);
